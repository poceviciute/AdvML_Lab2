...
We also use the Viterbi algortihm implemented in the HMM package to find the most probable path. The results are following:
...
#Q5
```{r}
#initial the vector for storing the accuracy rates
accuracy_filter <- c()
accuracy_smooth <- c()
accuracy_MPp <- c()
...
accuracy_MPp[i] <- 
  length(Most_Prob_path[Most_Prob_path==sim2$states])/length(sim2$states)
}
